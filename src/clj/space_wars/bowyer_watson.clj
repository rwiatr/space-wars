...
(defn- self-return [] self-return)

(defn bowyer-watson_2d [points & {:keys [boundries timer-fn]
                                  :or {boundries (triangle (point -100 -100) (point 100 -100) (point 0 100))
                                       timer-fn self-return}}]
  (loop [triangles #{boundries}
         points points
         timer-fn timer-fn]
    (if (empty? points) triangles ;; end of points - stop condition
      (let [point (first points)
            badTriangles (triangles-with-intersecting-circumcircle point triangles)
            boundries (boundry-edges badTriangles)
            newTriangles (recreate point boundries)]
        (recur (-> triangles
